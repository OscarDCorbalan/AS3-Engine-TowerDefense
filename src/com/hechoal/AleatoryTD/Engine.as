package com.hechoal.AleatoryTD {
	...
	import com.hechoal.AleatoryTD.SWFProfiler;
	import com.hechoal.AleatoryTD.UI.*;
	...
	import com.hechoal.GameJoltAPI;
	...
	public class Engine extends MovieClip{
		...
		public var poolP:gaObjectPool;
		private var poolY:gaObjectPool;
		private var poolC:gaObjectPool;
		private var poolM:gaObjectPool;
		...
		public function Engine(_ms:MovieClip, _la:Array, _map:uint, _kong:*, _dbl:Boolean = false) {
			...
			poolP = new gaObjectPool(Class(Particle),15);
			poolY = new gaObjectPool(Class(Triangulation), 1);
			poolC = new gaObjectPool(Class(GravityBullet), 5);
			poolM = new gaObjectPool(Class(Ray), 5);
		...
		private function destroyThis(won:Boolean = false){
			...
			else if(url.indexOf("kongregate.com")==-1 && url.indexOf("gamejolt.com")==-1 ){
				MochiScores.showLeaderboard({boardID: "d1ece1bfec139efe", score: uistats.getScore(), res:"550x550", width:400, height:400, onDisplay:stub, onClose:backtomenu});
			}
